  ...
  class TwineFile
    ...
    def traverse(node,level=0, path = "", parent_section = nil)
      node.each do |subnode|
        ...
        if (value.class == Hash)
          @sections[path] = TwineSection.new(path) unless @sections[path] or path.empty?
    ...
    def resolve_references()
      @definitions_by_key.each do |key, definition|
        definition.reference = @definitions_by_key[definition.reference_key] if definition.reference_key
      end
    end
    ...
    def read_old(path)
      ...
      definition.reference = @definitions[definition.reference_key] if definition.reference_key
    end
  end
